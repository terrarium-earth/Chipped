...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    private void createBlockSet(ResourcefulRegistry<Block> registry, String folder, String replace) {
        ...
        for (Block block : blocks) {
            basicBlockItem(block, new ResourceLocation(Chipped.MOD_ID, Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath().replace(replace, "")), folder);
        }
    }
    ...
    public void basicCustomItem(Block item, String folder) {
        basicItem(Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(item)), folder);
    }
