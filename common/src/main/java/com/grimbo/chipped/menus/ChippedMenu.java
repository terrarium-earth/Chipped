...
public class ChippedMenu extends AbstractContainerMenu {
	...
	public ChippedMenu(int id, Inventory inventory, RecipeType<ChippedRecipe> recipeType, Block block) {
		this(id, inventory, null, recipeType, ContainerLevelAccess.NULL, block);
	}

	public ChippedMenu(int id, Inventory inventory, MenuType<ChippedMenu> menuType, RecipeType<?> recipeType, ContainerLevelAccess access, Block block) {
		super(menuType, id);
	...
	public boolean stillValid(Player player) {
		return stillValid(this.access, player, this.blockWorkbench);
	}
	...
	private void setupRecipeList(Container container, ItemStack stack) {
		...
		if (!stack.isEmpty()) {
			this.recipe = this.level.getRecipeManager().getRecipeFor((RecipeType<ChippedRecipe>) this.recipeType, container, this.level).orElse(null);
			if (this.recipe != null) {
	...
	public ItemStack quickMoveStack(Player player, int i) {
		...
		if (slot != null && slot.hasItem()) {
			...
			} else if (this.level.getRecipeManager().getRecipeFor((RecipeType<ChippedRecipe>) this.recipeType, new SimpleContainer(itemStack2), this.level).isPresent()) {
